...
work_dir=paste0(getwd(),"/DEseq")
qc_dir=paste0(getwd(),"/DEseq/qc")
counts_dir=paste0(getwd(),"/rsem_out")
norm_output=paste0(getwd(),"/DEseq/norm_counts")
DGE_output=paste0(getwd(),"/DEseq/out")
...
png(filename = paste0(qc_dir,"/pca.png"),width = 700 ,height = 700)
plotPCA(rld, intgroup="condition")
dev.off()

rld_mat <- assay(rld)
rld_cor <- cor(rld_mat)
png(filename = paste0(qc_dir,"/heatmap.png"),width = 700 ,height = 700)
pheatmap(rld_cor)
dev.off()
...
for (i in 1:dim(contrasts)[2]){
	res_1 <- results(dds_1, contrast=c("condition",contrasts[1,i],contrasts[2,i]), alpha = 0.05)
